...
fk_query:
    ==FACTOR:10== fk_alter_table
  | ==FACTOR:5== fk_create_table
  | fk_set_checks
  | ==FACTOR:5== CHECK TABLE _table __extended(80)
;

fk_create_table:
     DROP TABLE IF EXISTS fk_child
  ;; CREATE OR REPLACE TABLE fk_parent (f1 _datatype[invariant], KEY(f1)) ENGINE=InnoDB
  ;; CREATE OR REPLACE TABLE fk_child (f2 _datatype[invariant], fk_constraint_optional FOREIGN KEY fk_index_name_optional (f2) REFERENCES fk_parent(f1) fk_optional_on_delete fk_optional_on_update) ENGINE=InnoDB
  ;; INSERT IGNORE INTO fk_parent VALUES (),(),(),(),()
  ;; INSERT IGNORE INTO fk_child VALUES (),(),(),(),()
   | DROP TABLE IF EXISTS fk_child
  ;; CREATE OR REPLACE TABLE fk_parent (f1 _datatype[invariant], f2 INT, KEY(f1,f2)) ENGINE=InnoDB
  ;; CREATE OR REPLACE TABLE fk_child (f3 _datatype[invariant], f4 INT, fk_constraint_optional FOREIGN KEY fk_index_name_optional (f3,f4) REFERENCES fk_parent(f1,f2) fk_optional_on_delete fk_optional_on_update) ENGINE=InnoDB
  ;; INSERT IGNORE INTO fk_parent VALUES (),(),(),(),()
  ;; INSERT IGNORE INTO fk_child VALUES (),(),(),(),()
;
...
fk_index_name_optional:
  | { 'fk'.$prng->uint16(1,200) }
;

fk_drop_foreign_key:
  DROP FOREIGN KEY fk_if_exists { 'fk'.$prng->uint16(1,200) }
;
