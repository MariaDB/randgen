...
#  $options{optional_ps_protocol},
  ...
  [ '--scenario=Replication --scenario-primary-crash-recovery --filter=conf/preview/new_binlog.ff --filter=conf/ff/replication.ff --engine=InnoDB --mysqld=--enforce-storage-engine=InnoDB --nometadata-reload' ],
