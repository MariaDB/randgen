...
query_init:
  ...
  EXECUTE IMMEDIATE CONCAT('REVOKE ALTER, DROP ON partition_by_columns_db.* FROM ',CURRENT_USER);
...
partition_pruning:
  | | | PARTITION (p0) | PARTITION (p1) | PARTITION (p0,p1) | PARTITION (p1,p0) ;
...
utf8_char_select_count3:
  SELECT COUNT(*) FROM _table[invariant] partition_pruning WHERE `col_varchar_5_utf8` = _char[invariant];
...
new_table_item:
  _table partition_pruning AS { "table".++$tables };
