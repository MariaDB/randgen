...
#          if ($client and
#          if (system("$client --force -uroot --protocol=tcp --port=".$executor->port()." ".$executor->defaultSchema()." < $spec_file") == STATUS_OK)
          if (system("perl $ENV{RQG_HOME}/populate-schema.pl --dsn=".$self->dsn." --rows=5000 --basedir=".$executor->serverVariable('basedir')." --schema=$spec_file") == STATUS_OK)
          {
