...
sub configure {
	...
	if (defined $list and $#{$list} >= 0) {
		...
		sayWarning("No transformers were defined to be run by TransformerLight validator");
    return;
	}

	say("TransformerLight validator will use the following transformers: ".join(', ', @transformer_names));
...
sub validate {
	...
	foreach my $transformer (@transformers) {
        if (time() > $executor->end_time) {
            say("TransformerLight: Test duration has already been exceeded, exiting");
            last;
        }
