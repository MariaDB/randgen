...
use constant  ER_WRONG_FIELD_TERMINATORS                        => 1083;
...
use constant  ER_UNKNOWN_CHARACTER_SET                          => 1115;
use constant  ER_TOO_BIG_ROWSIZE                                => 1118;
use constant  ER_STACK_OVERRUN                                  => 1119;
use constant  ER_WRONG_OUTER_JOIN                               => 1120;
use constant  ER_UDF_NO_PATHS                                   => 1124;
...
use constant  ER_ILLEGAL_GRANT_FOR_TABLE                        => 1144;
...
use constant  ER_CHECK_NO_SUCH_TABLE                            => 1177;
...
use constant  ER_BAD_SLAVE                                      => 1200;
...
use constant  ER_VARIABLE_IS_NOT_STRUCT                         => 1272;
use constant  ER_UNKNOWN_COLLATION                              => 1273;
use constant  ER_BAD_SLAVE_UNTIL_COND                           => 1277;
...
use constant  ER_SP_LILABEL_MISMATCH                            => 1308;
...
use constant  ER_SP_BAD_CURSOR_SELECT                           => 1323;
use constant  ER_SP_CURSOR_MISMATCH                             => 1324;
use constant  ER_SP_UNDECLARED_VAR                              => 1327;
...
use constant  ER_VIEW_SELECT_CLAUSE                             => 1350;
...
use constant  ER_SP_BAD_SQLSTATE                                => 1407;
...
use constant  ER_TOO_BIG_PRECISION                              => 1426;
...
use constant  ER_FOREIGN_SERVER_DOESNT_EXIST                    => 1477;
...
use constant  ER_WRONG_SIZE_NUMBER                              => 1531;
...
use constant  ER_BASE64_DECODE_ERROR                            => 1575;
...
use constant  ER_SLAVE_HEARTBEAT_VALUE_OUT_OF_RANGE             => 1624;
...
use constant  ER_UNKNOWN_EXPLAIN_FORMAT                         => 1791;
...
use constant  ER_JSON_BAD_CHR                                   => 4035;
use constant  ER_JSON_NOT_JSON_CHR                              => 4036;
use constant  ER_JSON_EOS                                       => 4037;
use constant  ER_JSON_SYNTAX                                    => 4038;
use constant  ER_JSON_ESCAPING                                  => 4039;
use constant  ER_JSON_DEPTH                                     => 4040;
use constant  ER_JSON_PATH_EOS                                  => 4041;
use constant  ER_JSON_PATH_SYNTAX                               => 4042;
use constant  ER_JSON_PATH_DEPTH                                => 4043;
use constant  ER_JSON_PATH_NO_WILDCARD                          => 4044;
use constant  ER_JSON_PATH_ARRAY                                => 4045;
use constant  ER_JSON_ONE_OR_ALL                                => 4046;
...
use constant  ER_ILLEGAL_PARAMETER_DATA_TYPE_FOR_OPERATION      => 4079;
use constant  ER_WRONG_PARAMCOUNT_TO_CURSOR                     => 4080;
use constant  ER_UNKNOWN_STRUCTURED_VARIABLE                    => 4081;
...
use constant  ER_UNKNOWN_COMPRESSION_METHOD                     => 4098;
use constant  ER_NOT_AGGREGATE_FUNCTION                         => 4105;
...
# 10.5+ errors (these codes can still change)
...
use constant  ER_WITH_TIES_NEEDS_ORDER                          => 4180;
use constant  ER_REMOVED_ORPHAN_TRIGGER                         => 4181;
use constant  ER_STORAGE_ENGINE_DISABLED                        => 4182;
...
my %err2type = (
    ...
    ER_BAD_SLAVE()                                      => STATUS_SEMANTIC_ERROR,
    ...
    ER_ILLEGAL_GRANT_FOR_TABLE()                        => STATUS_SEMANTIC_ERROR,
    ...
    ER_JSON_TABLE_MULTIPLE_MATCHES()                    => STATUS_SEMANTIC_ERROR,
    ER_JSON_TABLE_SCALAR_EXPECTED()                     => STATUS_SEMANTIC_ERROR,
    ...
    ER_REMOVED_ORPHAN_TRIGGER()                         => STATUS_SEMANTIC_ERROR,
    ...
    ER_SP_BAD_CURSOR_SELECT()                           => STATUS_SEMANTIC_ERROR,
    ER_SP_BAD_SQLSTATE()                                => STATUS_SEMANTIC_ERROR,
...
#    ER_SP_PROC_TABLE_CORRUPT()                          => STATUS_DATABASE_CORRUPTION,  # this error is bogus due to bug # 47870
    ...
    ER_UNKNOWN_EXPLAIN_FORMAT()                         => STATUS_SEMANTIC_ERROR,
...
#    ER_UNKNOWN_STORAGE_ENGINE()                         => STATUS_ENVIRONMENT_FAILURE,
    ...
    ER_WRONG_TABLE_NAME()                               => STATUS_SEMANTIC_ERROR,
...
sub execute {
    ...
    #
    # First check the query for compatibility and incompatibility markers
    # /* compatibility X.Y.Z, A.B.C */ and similar markers are set when a query
    # requires a server version X.Y.Z or higher or A.B.C or higher
    # /* incompatibility X.Y.Z, A.B.C */ and similar markers are set when
    # a query is no longer applicable starting from X.Y.Z and A.B.C
    # There can be more than one marker of each kind in the resulting query, e.g.
    # ALTER ... /* compatibility 10.2.37, 10.3.18 */ ... RENAME /* compatibility 10.2.41 */, ALGORITHM ... /* incompatibility 10.4.19, 10.5.5 */, LOCK ... /* incompatibility 10.5.10 */
    #
    # For the query to be compatible, each compatibility marker (each comment)
    # should have at least one match. That is, for the above example
    # the server version should be (ver >= 10.2.37 OR ver >= 10.3.18) AND (ver >= 10.2.41)
    # Thus, 10.2.41, 10.3.18 and 10.4.0 are compatible; 10.2.38 is not.
    #
    # For the query to be incompatible, it should either break compatibility rules above,
    # or there should be at least one incompatibility marker (comment) with a match.
    # For the above example the query is skipped if the server version is
    # (ver >= 10.4.19 OR ver >= 10.5.5) OR (ver >= 10.5.10).
    # Thus, 10.3.29, 10.4.18 and 10.5.4 are compatible, 10.4.19 and 10.5.6 are not.
    #

    # First check for incompatibilities
    my @incompat_requirements= $query=~ /\/\*\s*incompatibility\s+([\d\.]+(?:e|-[0-9]+)?(?:,\s*[\d\.]+(?:e|-[0-9]+)?)*)\s*\*\//g;
    my $incompat;
    INCOMPAT:
    foreach my $ir (@incompat_requirements) {
        my @ir= split /,/, $ir;
        $incompat= 0;
        foreach my $i (@ir) {
            $incompat= $executor->is_compatible($i);
            last INCOMPAT if $incompat;
        }
    }
    # If no incompatibility detected, check compatibility rules
    unless ($incompat) {
      my @compat_requirements= $query=~ /\/\*\s*compatibility\s+([\d\.]+(?:e|-[0-9]+)?(?:,\s*[\d\.]+(?:e|-[0-9]+)?)*)\s*\*\//g;
    ...
    if ($incompat) {
      ...
      sayDebug("Query $query does not meet compatibility requirements");
