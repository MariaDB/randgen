...
sub run {
    ...
    if ($schema_file) {
        ...
        system("LD_LIBRARY_PATH=\$MSAN_LIBS:\$LD_LIBRARY_PATH $mysql_client_path --no-defaults --init-command='/*!100105 SET tx_read_only=OFF */' --port=$port --protocol=tcp -uroot --force ".$self->db." < $schema_file");
        if ($?) {
