...
sub run {
    ...
    if ($schema_file) {
        ...
        system("LD_LIBRARY_PATH=\$MSAN_LIBS:\$LD_LIBRARY_PATH $mysql_client_path --init-command='SET tx_read_only=OFF' --port=$port --protocol=tcp -uroot --force ".$self->db." < $schema_file");
        if ($?) {
